...
class ModelManager:
    ...
    async def _get_async_lock(self):
        """Get or create async lock with proper error handling."""
        if self._async_lock is None:
            try:
                self._async_lock = asyncio.Lock()
            except RuntimeError:
                # Event loop not available, create it when needed
                pass
    ...
    async def initialize(self, force_reload: bool = False) -> bool:
        ...
        try:
            ...
            await self._get_async_lock()
            
            # Initialize Ollama client with timeout protection
            ...
            if not hasattr(self, 'initialization_status') or self.initialization_status == "not_started":
    ...
    async def _discover_available_models(self) -> None:
        """Discover and catalog available models from Ollama with timeout protection."""
        try:
            ...
            for model_data in models_data:
                try:
                    ...
                    logger.warning(f"Failed to process model data {model_data}: {model_error}")
                    continue
    ...
    async def generate(
        ...
        **kwargs
        ...
        """
        Generate text using the specified model with timeout protection.
        ...
        try:
            ...
            self.cost_tracker[model_name] = self.cost_tracker.get(model_name, 0) + getattr(result, 'cost', 0)
            
            return result
            
        except asyncio.TimeoutError:
            logger.error(f"Generation timed out for model {model_name}")
            return ModelResult(
                success=False,
                text="",
    ...
    async def shutdown(self):
        """Gracefully shutdown the model manager with resource cleanup."""
        ...
        try:
            if self.ollama_client:
                try:
                    ...
                    logger.warning("Ollama client shutdown timed out")
                except Exception as e:
    ...
    def get_stats(self) -> Dict[str, Any]:
        """Get overall performance and usage statistics."""
        return {
            "total_models": len(self.models),
            "available_models": len([m for m in self.models.values() if m.status == ModelStatus.READY]),
            "total_requests": sum(self.usage_stats.values()) if self.usage_stats else 0,
            "total_cost": sum(self.cost_tracker.values()) if self.cost_tracker else 0,
